...
_check-version:
    #!/usr/bin/env bash
    set -euo pipefail
    cabal_v=$(awk '/^version:/{print $2; exit}' graded-monads.cabal)
...
_require-unreleased:
    #!/usr/bin/env bash
    set -euo pipefail
    version=$(awk '/^version:/{print $2; exit}' graded-monads.cabal)
...
release VERSION: _require-clean-main
    ...
    sed -i -E "s/^(version:[[:space:]]*).*/\1${version}/" graded-monads.cabal
...
publish-candidate: _require-clean-main _check-version
    ...
    version=$(awk '/^version:/{print $2; exit}' graded-monads.cabal)
    cabal sdist all
    sdist="dist-newstyle/sdist/graded-monads-${version}.tar.gz"
    [ -f "$sdist" ] || { echo "error: sdist not found at $sdist" >&2; exit 1; }
    cabal upload "$sdist"
    echo "Uploaded candidate. Review: https://hackage.haskell.org/package/graded-monads-${version}/candidate"
...
publish: _require-clean-main _check-version _require-unreleased test
    ...
    echo "Published graded-monads-${version} to Hackage and pushed tag ${version}."
